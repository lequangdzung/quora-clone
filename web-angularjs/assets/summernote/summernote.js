...
(function (factory) {
  ...
  var settings = {
    ...
    options: {
      ...
      onAfterImageInsert: null,      // imageUpload
      onAfterVideoInsert: null,      // imageUpload
  ...
  var Editor = function () {
    ...
    this.insertImage = function ($editable, sUrl) {
      ...
      async.createImage(sUrl).then(function ($image) {
        ...
        obj.restoreRange($editable);
        range.create().insertNode($image[0]);
        var callbacks = $editable.data('callbacks');
        if (callbacks.onAfterImageInsert) {
          callbacks.onAfterImageInsert(sUrl, $editable);
        }
      }).fail(function () {
        var callbacks = $editable.data('callbacks');
     ...
     * @param {String} sUrl
     */
    this.insertImageSmart = function ($editable, sUrl, w, h) {
      this.saveRange($editable);
      var obj = this;
      async.createImage(sUrl).then(function ($image) {
        recordUndo($editable);
    ...
    this.insertVideo = function ($editable, sUrl) {
      ...
      if ($video) {
        ...
        var callbacks = $editable.data('callbacks');
        if (callbacks.onAfterVideoInsert) {
          callbacks.onAfterVideoInsert(provider, videoId, sUrl, $video[0], $editable);
        }
      }
    };
  ...
  var EventHandler = function () {
      ...
      showImageDialog: function (oLayoutInfo) {
        ...
        dialog.showImageDialog($editable, $dialog).then(function (data) {
          ...
          if (typeof data === 'string') {
            ...
            console.log("data is mutple files: ", data);
            // array of files
    ...
    this.attach = function (oLayoutInfo, options) {
      ...
      oLayoutInfo.editable.data('callbacks', {
        ...
        onAfterImageInsert: options.onAfterImageInsert,
        onAfterVideoInsert: options.onAfterVideoInsert,
    ...
    var tplDialogs = function (lang, options) {
      var tplImageDialog = function () {
        var body = '<h5>' + lang.image.selectFromFiles + '</h5>' +
                   ...
                   '<h5 class="label-image-url">' + lang.image.url + '</h5>' +
