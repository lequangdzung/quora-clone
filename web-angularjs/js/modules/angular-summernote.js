...
angular.module('summernote', [])
  ...
  .controller('SummernoteController', ['$scope', '$attrs', function($scope, $attrs) {
    ...
    // if (angular.isDefined($attrs.onAfterImageInsert)) {
    //   console.log("come here");
    //   summernoteConfig.onAfterImageInsert = function(sDataURL, editor, welEditable) {
    //     $scope.afterImageInsert({sDataURL:sDataURL, editor:editor, welEditable:welEditable});
    //   };
    // }
    ...
    this.activate = function(scope, element, ngModel) {
      ...
      if(angular.isDefined(summernoteConfig.onAfterImageInsert)) {
        var oldImageFnc = summernoteConfig.onAfterImageInsert;
        summernoteConfig.onAfterImageInsert = function(sDataURL, $editable) {
          console.log("called oldFnc first:", oldImageFnc);
          oldImageFnc(sDataURL, $editable);
          updateNgModel();
        };
      }
      else {
        summernoteConfig.onAfterImageInsert = function(sDataURL, $editable) {
          console.log("working on updateNgModel:", $editable);
          updateNgModel();
        };
      }

      if(angular.isDefined(summernoteConfig.onAfterVideoInsert)) {
        var oldVideoFnc = summernoteConfig.onAfterVideoInsert;
        summernoteConfig.onAfterVideoInsert = function(provider, videoId, url, video, $editable) {
          oldVideoFnc(provider, videoId, url, video, $editable);
          updateNgModel();
        };
      }
      else {
  ...
  .directive('summernote', [function() {
    ...
    return {
      ...
      scope: {
        ...
        afterImageInsert: '&onAfterImageInsert'
      },
